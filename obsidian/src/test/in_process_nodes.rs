...
pub(crate) struct InProcessNodes {
    inner: Arc<InProcessNodesInner>,
    discovery: Arc<Discovery>,
}

struct InProcessNodesInner {
...
impl InProcessNodes {
    pub fn new(node_builder: Box<dyn TestNodeBuilder>) -> Self {
        let inner = Arc::new(InProcessNodesInner {
            node_builder,
...
#[async_trait]
impl TestNodes for InProcessNodes {
...
impl NodeDiscovery for InProcessNodesInner {
    fn node_ids(&self) -> (OrdSet<NodeId>, Box<dyn Future<Output = ()> + Send + Unpin>) {
